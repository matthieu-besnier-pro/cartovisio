...
export default function PublicMapEditor() {
  ...
  // Public save: offload to a backend function (service role) since the viewer is unauthenticated.
  // The editor passes raw JSON (serializeOnly); the function uploads large fields and stores URLs.
  const handleSave = async (data) => {
    const res = await base44.functions.invoke('savePublicMap', { shareToken: token, ...data });
  ...
  return <CommercialMapEditor record={record} readOnly={false} serializeOnly onSave={handleSave} />;
}